...
def plataform():
    ...
    st.write('Essa plataforma de Machine learning foi desenvolvida para resolver está dor do Agricultor brasileiro, na plataforma ele consegue fazer a previsão da colheita da Safra.  \
             A plataforma é armazenada na nuvem e está disponível para uso do agricultor, permitindo que ele tenha acesso apenas entrando neste aplicativo web e  através do preenchimento dos dados, \
             em segundos a inferência é feita ao modelo de machine learning,  e ele terá a previsão em mãos sobre o resultado da colheita de safra.')
...
def safra():
    ...
    if st.button('Predição'):
        ...
        if output == 0:
            st.success('Colheita de Safra:  {}'.format('Sem danos'))
        elif output == 1:
            st.success('Colheita de Safra:  {}'.format(
