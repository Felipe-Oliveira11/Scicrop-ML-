...
def safra():
    ...
    if st.button('Predição'):
        ...
        if output == 0:
            st.success('Predição: Colheita de Safra {}'.format('Sem danos'))
        elif output == 1:
            st.success('Predição: {}'.format('Danos por outros motivos'))
        else:
            st.success('Predicção: {}'.format('Danos gerados por pesticidas'))
